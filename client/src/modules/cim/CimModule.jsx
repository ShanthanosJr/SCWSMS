...
function CimModule(){
  ...
  const logout = () => { try{ localStorage.removeItem('token'); localStorage.removeItem('user'); }catch{}; setUser(null); window.location.hash = '#/Login' }
    ...
    <Routes>
      <Route path="/" element={
        <>
          ...
          {!allowed.includes(page) && (window.location.hash = `#/${allowed[0]}`)}
