...
const calculateLaborCosts = (workers, engineers, architects, projectManagers) => {
  ...
  // Calculate project manager costs
  ...
  materials.forEach(material => {
    if (material && material.name && material.quantity) {
      ...
      if (materialConfig) {
        ...
        // cost if no predefined cost available
        ...
        // cost if no configuration and no cost provided
...
const updateDashboard = async (req, res) => {
  try {
    ...
    // If not found  try to find by dashboardId field
    if (!dashboard) {
...
module.exports = {
  ...
  exportDashboard  // export function to exports
};