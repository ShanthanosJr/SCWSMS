...
export default function ProjectReqsAll() {
    ...
    const [notifications, setNotifications] = useState([
        {
            ...
            message: 'Project Requests Nexus is ready for inquiry management',
            timestamp: new Date(),
            icon: '📋',
            read: false
        }
    ...
    useEffect(() => {
        ...
        (async () => {
            setLoading(true);
            const previousReqCount = projectReqs.length;
            const data = await fetchProjectReqs();
            if (!mounted) return;
            
            // Check for new project requests
            if (previousReqCount > 0 && data.length > previousReqCount) {
                const newReqsCount = data.length - previousReqCount;
                addNotification(
                    'addition',
                    `New Project Request${newReqsCount > 1 ? 's' : ''} Received`,
                    `${newReqsCount} new inquiry${newReqsCount > 1 ? ' has' : ' have'} been submitted`,
                    '📋'
                );
            }
    ...
    // Auto-refresh project requests every 30 seconds to catch new additions/updates
    useEffect(() => {
        const interval = setInterval(async () => {
            try {
                const newProjectReqs = await fetchProjectReqs();
                
                // Check for new requests by comparing IDs
                const currentReqIds = projectReqs.map(r => r._id);
                const addedReqs = newProjectReqs.filter(r => !currentReqIds.includes(r._id));
                
                if (addedReqs.length > 0) {
                    addedReqs.forEach(req => {
                        addNotification(
                            'addition',
                            'New Project Request Received',
                            `Request from ${req.preqname || 'Unknown Client'} has been submitted`,
                            '📋'
                        );
                    });
                    setProjectReqs(newProjectReqs);
                }
            } catch (error) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm("⚠️ Are you sure you want to delete this project request? This action cannot be undone.")) {
            try {
                // Get request info before deletion for notification
                const requestToDelete = projectReqs.find(r => r._id === id);
                const clientName = requestToDelete?.preqname || 'Unknown Client';
                ...
                addNotification(
                    ...
                    `Request from ${clientName} has been removed from the system`,
                    '🗑️'
                );
                ...
                alert.innerHTML = `
                    <div class="d-flex align-items-center">
                        <div class="me-2">✅</div>
                        <div><strong>Success!</strong> Project request deleted successfully.</div>
                ...
                alert.innerHTML = `
                    <div class="d-flex align-items-center">
                        <div class="me-2">❌</div>
                        <div>Error deleting project request. Please try again.</div>
                        <button type="button" class="btn-close ms-auto" data-bs-dismiss="alert"></button>
                    </div>
                `;
                document.body.appendChild(alert);
    ...
    // Handle view request with notification
    const handleViewRequest = (req) => {
        // Add view notification
        addNotification(
            'view',
            'Project Request Viewed',
            `Request from ${req.preqname || 'Unknown Client'} has been accessed`,
            '👁️'
        );
                        ...
                        <button
                            ...
                            style={{
                               ...
                               // border: 'none',
                                ...
                                cursor: 'pointer',
                                position: 'relative',
                                boxShadow: '0 8px 25px rgba(146, 64, 14, 0.4), 0 4px 12px rgba(146, 64, 14, 0.2)',
                            ...
                            onMouseEnter={(e) => {
                                ...
                                e.currentTarget.style.boxShadow = '0 12px 35px rgba(146, 64, 14, 0.6), 0 6px 16px rgba(146, 64, 14, 0.3)';
                            }}
                            onMouseLeave={(e) => {
                                e.currentTarget.style.transform = 'translateY(0) scale(1)';
                                e.currentTarget.style.boxShadow = '0 8px 25px rgba(146, 64, 14, 0.4), 0 4px 12px rgba(146, 64, 14, 0.2)';
                            }}
                        >
                            <BsBellFill
                                size={26}
                        ...
                        {showNotifications && (
                            <div
                                style={{
                                    ...
                                    borderRadius: '20px',
                                    boxShadow: '0 20px 60px rgba(146, 64, 14, 0.15), 0 8px 25px rgba(0, 0, 0, 0.1)',
                            ...
                            >
                                ...
                                >
                                    <div className="d-flex align-items-center justify-content-between">
                                        <h6 className="mb-0 fw-bold" style={{ color: '#78350f', fontSize: '1.1rem' }}>
                                        ...
                                        <div className="d-flex align-items-center gap-2">
                                            ...
                                            {notificationCount > 0 && (
                                                <button
                                                    ...
                                                    onMouseEnter={(e) => {
                                                        e.target.style.background = 'rgba(146, 64, 14, 0.1)';
                                                    }}
                                                    onMouseLeave={(e) => {
                                    ...
                                    {notifications.length === 0 ? (
                                        ...
                                        notifications.map((notification) => {
                                            const getNotificationGradient = (type) => {
                                                switch (type) {
                                                    case 'addition': return 'linear-gradient(135deg, #92400e 0%, #b45309 100%)';
                                                    case 'view': return 'linear-gradient(135deg, #a16207 0%, #ca8a04 100%)';
                                                    case 'update': return 'linear-gradient(135deg, #dc2626 0%, #b91c1c 100%)';
                                                    ...
                                                    default: return 'linear-gradient(135deg, #b45309 0%, #78350f 100%)';
                                                }
                                            };
                                            ...
                                            return (
                                                <div 
                                                    ...
                                                    style={{ 
                                                        padding: '1.5rem 2rem',
                                                        borderBottom: notifications.indexOf(notification) === notifications.length - 1 ? 'none' : '1px solid rgba(146, 64, 14, 0.05)',
                                                        cursor: 'pointer',
                                                        transition: 'background 0.15s ease',
                                                        background: notification.read ? 'transparent' : 'rgba(146, 64, 14, 0.02)',
                                                        borderLeft: notification.read ? 'none' : '3px solid #92400e'
                                                    }}
                                                    onMouseEnter={(e) => e.currentTarget.style.background = '#fef3e2'}
                                                    onMouseLeave={(e) => e.currentTarget.style.background = notification.read ? 'transparent' : 'rgba(146, 64, 14, 0.02)'}
                            ...
                            <div className="text-center mb-5" style={{
                                ...
                                <div className="d-flex justify-content-center gap-3 flex-wrap">
                                    ...
                                    <button onClick={() => navigate("/ptfd/financial-dashboard")} className="btn btn-outline-primary btn-lg px-5 py-3 fw-semibold" style={{
                                        ...
                                        <BsCalculator className="me-2" />View Dashboards
                                    </button>
                                </div>
                            </div>
                        </div>
