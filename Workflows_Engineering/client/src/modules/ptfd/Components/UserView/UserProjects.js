...
const UserProjects = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'preqdescription') {
      const textarea = e.target;
      textarea.style.height = 'auto';
      const scrollHeight = textarea.scrollHeight;
      const maxHeight = 200; // matches CSS max-height
      const minHeight = 120; // matches CSS min-height
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      setFormMessage("⚠️ Please correct the highlighted fields before submitting.");
      return;
    }
    ...
    try {
      ...
      setFormData({ preqname: "", preqmail: "", preqnumber: "", preqdescription: "" });
      setErrors({});
    ...
    switch (name) {
      ...
      case "preqnumber":
        return /^(?:\+94\d{9}|0\d{9})$/.test(value)
          ? ""
          : "Enter a valid phone number (+94XXXXXXXXX or 0XXXXXXXXX)";
      case "preqdescription":
        ...
        <div className="premium-container">
          <div className="premium-about-grid">
            <div className="premium-about-content">
              ...
              <button onClick={() => navigate("/ptfd/join-with-us")} className="premium-btn premium-btn-primary">Learn More About Us</button>
            </div>
              ...
              <div className="premium-form-row">
                <div className="form-field">
                  <input
                    ...
                    className={`premium-form-input ${errors.preqname ? "input-error" : ""}`}
                    value={formData.preqname}
                    onChange={handleInputChange}
                    required
                  />
                  {errors.preqname && <span className="error-text">{errors.preqname}</span>}
                </div>
                <div className="form-field">
                  <input
                    type="email"
                    ...
                    className={`premium-form-input ${errors.preqmail ? "input-error" : ""}`}
                    value={formData.preqmail}
                    onChange={handleInputChange}
                    required
                  />
                  {errors.preqmail && <span className="error-text">{errors.preqmail}</span>}
                </div>
              </div>
              <div className="form-field">
                <input
                  ...
                  className={`premium-form-input ${errors.preqnumber ? "input-error" : ""}`}
                  value={formData.preqnumber}
                  onChange={handleInputChange}
                  required
                />
                {errors.preqnumber && <span className="error-text">{errors.preqnumber}</span>}
              </div>
              <div className="form-field">
                <textarea
                  ref={textareaRef}
                  ...
                  rows="4"
                  className={`premium-form-textarea ${errors.preqdescription ? "input-error" : ""}`}
                  value={formData.preqdescription}
                  onChange={handleInputChange}
                  required
                ></textarea>
                {errors.preqdescription && <span className="error-text">{errors.preqdescription}</span>}
              </div>
              <button
                type="submit"
